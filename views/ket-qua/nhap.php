            ...
            <div class="card-body">
                <form action="/ket-qua/luu" method="POST">
                    ...
                    <div class="row mb-4 bg-light p-3 rounded border align-items-center">
                        <div class="col-md-4">
                            <p class="mb-1">Mã phiếu YC: <strong class="text-danger"><?= htmlspecialchars($phieu['pyc_so_phieu'] ?? 'Không xác định') ?></strong></p>
                        </div>

                        <div class="col-md-4 d-flex align-items-center justify-content-end">
                            <label class="mb-0 me-2 fw-bold text-nowrap">Ngày nhận mẫu</label>
                            <input type="text" name="ngay_nhan_mau" class="form-control" style="max-width: 200px;" value="<?= $phieu['ngay_nhan_mau'] ?? date('d-m-Y') ?>">
                        </div>
                        <div class="col-md-4 d-flex align-items-center justify-content-end">
                            <label class="mb-0 me-2 fw-bold text-nowrap">Ngày thí nghiệm:</label>
                            <input type="date" name="ngay_thi_nghiem" class="form-control" style="max-width: 200px;" value="<?= $phieu['ngay_thi_nghiem'] ?? date('Y-m-d') ?>">
                        </div>
                    </div>
                                        ...
                                        <table class="table table-bordered table-tho mb-4 phep-thu-table" data-cong-thuc="<?= htmlspecialchars($pt['pt_cong_thuc'] ?? '') ?>">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th width="15%">Kết quả ( <?= htmlspecialchars($pt['pt_don_vi'] ?? '') ?> )</th>
                                                            ...
                                                            <?php foreach ($pt['danh_sach_truong'] as $truong): ?>
                                                                <td>
                                                                    <input type="number" step="any" class="input-tho var-input" data-var="<?= htmlspecialchars($truong['cht_ten_bien']) ?>" required
                                                                        ...
                                                                        value="<?= htmlspecialchars($duLieuTho[$v][$truong['cht_ten_bien']] ?? '') ?>">
                                                                </td>
                                                            <?php endforeach; ?>

                                                            <td>
                                                                <input type="text" class="input-tho readonly-input kq-mau text-primary" readonly tabindex="-1"
                                                                    name="du_lieu[<?= $mau['mtn_ma'] ?>][<?= $pt['pt_ma'] ?>][<?= $v ?>][kq_mau]"
                                                                    value="<?= htmlspecialchars($duLieuTho[$v]['kq_mau'] ?? '') ?>">
                                                            </td>

                                                            <td>
                                                                <input type="text" class="input-tho" placeholder="Ghi chú..."
                                                                    name="du_lieu[<?= $mau['mtn_ma'] ?>][<?= $pt['pt_ma'] ?>][<?= $v ?>][ghi_chu]"
                                                                    value="<?= htmlspecialchars($duLieuTho[$v]['ghi_chu'] ?? '') ?>">
                                                            </td>
                                                        </tr>
                                                    <?php endfor; ?>
                                                    ...
                                                    <tr class="row-ket-qua">
                                                        ...
                                                        <td>
                                                            <input type="text" class="input-tho readonly-input text-danger fw-bold kq-tong" readonly
                                                                ...
                                                                value="<?= htmlspecialchars($pt['kq_ket_luan'] ?? '') ?>">
                                                        </td>
                                                        <td></td>
                                                    </tr>
                                                <?php else: ?>
                                                    <tr>
                                                        <td colspan="4" class="text-center text-muted">Vui lòng thêm cấu hình trường cho phép thử trong CSDL.</td>
                                                    </tr>
                                                <?php endif; ?>
                                            </tbody>
                                        </table>
